...
def merge_news_and_stock(news_file, stock_file):
    ...
    # Sort dataframes by their respective date fields
    news_df = news_df.sort_values('published_at')
    stock_df = stock_df.sort_values('Date')

    # Merge the news_df with stock_df based on published_at and Date, allowing for tolerance
    merged_df = pd.merge_asof(
        news_df,
        stock_df,
        left_on='published_at',
        right_on='Date',
        direction='backward',  # Can be 'forward' or 'backward' depending on how you want to align the data
